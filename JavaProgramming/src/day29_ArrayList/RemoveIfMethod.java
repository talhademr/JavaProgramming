...
public class RemoveIfMethod {
    public static void main(String[] args) {
        ...
        list.addAll(Arrays.asList(1,2,3,4,5,7,8,9));
        list.addAll(Arrays.asList(1,2,3,4,5,7,8,9));
        list.addAll(Arrays.asList(1,2,3,4,5,7,8,9));
       ...
       /*
        for (int i = 0; i < list.size(); i++) {
            ...
            if (list.get(i) %2 !=0){    // bu methodu kullandığımızda kesin sonuç alamayız. çünkü arraylistdeki indexler değişiyor sürekli
                list.remove(i);
            }
        }*/
        System.out.println(list);

        list.removeIf( p -> p<5 ); // bunu ilerde görücez. Lambda expression
